...
export async function loadTrendingPostsForUser(
  ...
  const rawPosts = await prisma.$queryRaw<BigPostEntity[]>`
    ...
    LEFT JOIN "Tag" ON "Tag"."id" = "_PostToTag"."B"
    GROUP BY "Post"."id", "User"."id"
    ORDER BY "Post"."createdAt" DESC, popularity DESC
    OFFSET ${page * limit} ROWS
    FETCH NEXT ${limit} ROWS ONLY
  `;
  return rawPosts;
...
export async function loadFollowingPostsForUser(
  currentUserId: string,
  page: number
) {
  const limit = 10;
...
export async function loadFavouritePostsForUser(
  currentUserId: string,
  page: number
) {
  const limit = 10;
  ...
  const rawPosts = await prisma.$queryRaw<BigPostEntity[]>`
    ...
    FROM "Post"
    LEFT JOIN "User" ON "Post"."userId" = "User"."id"
    LEFT JOIN "Like" ON "Like"."userId" = ${currentUserId} AND "Like"."postId" = "Post"."id"
    LEFT JOIN "_PostToTag" ON "Post"."id" = "_PostToTag"."A"
    LEFT JOIN "Tag" ON "Tag"."id" = "_PostToTag"."B"
